...
from llava.model.language_model.llava_qwen_spatial import (
    LlavaQwenSpatialForCausalLM,
    LlavaQwenSpatialConfig,
)
local_rank = None
...
class DataArguments:
    ...
    eval_data_path: Optional[str] = field(default=None, metadata={"help": "Path to validation JSON for on-the-fly eval"})
    val_image_folder: Optional[str] = field(default=None, metadata={"help": "Folder with validation images (defaults to image_folder if None)"})
...
def find_all_linear_names(model):
    ...
    multimodal_keywords = ['mm_projector', 'vision_tower', 'spatial_tower', 'fusion_block']
...
class LazySupervisedDataset(Dataset):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if 'image' in sources[0]:
            ...
            image_folder = self.data_args.image_folder if self.train else self.data_args.val_image_folder
...
def make_supervised_data_module(tokenizer: transformers.PreTrainedTokenizer, data_args) -> Dict:
    ...
    if data_args.eval_data_path is not None:
        eval_dataset = LazySupervisedDataset(tokenizer=tokenizer, data_path=data_args.eval_data_path, data_args=data_args, train=False)
...
def get_model(model_args, training_args, bnb_model_from_pretrained_args):
    ...
    cfg_pretrained = None
    #cfg_pretrained = transformers.AutoConfig.from_pretrained(model_args.model_name_or_path)
    overwrite_config = {}
    ...
    if model_args.vision_tower is not None:
        if "mixtral" in model_args.model_name_or_path.lower():
            ...
            model = LlavaQwenSpatialForCausalLM.from_pretrained(
                model_args.model_name_or_path,
                cache_dir=training_args.cache_dir,
                attn_implementation=training_args.attn_implementation,
                torch_dtype=(torch.bfloat16 if training_args.bf16 else None),
            ...
            or "v1.5" in model_args.model_name_or_path.lower()
...
def train(attn_implementation=None):
    ...
    if model_args.vision_tower is not None:
        ...
        if model_args.mm_tunable_parts is None:  # traditional way of deciding which part to train
            ...
            if model_args.tune_mm_mlp_adapter or model_args.tune_fusion_block or model_args.tune_spatial_tower:
    ...
    #print(len(data_module['eval_dataset']))
    ...
    if training_args.lora_enable:
        ...
        if training_args.local_rank == 0 or training_args.local_rank == -1:
            ...
            if hasattr(model, "generation_config"):
                ...
                if not gc.do_sample:
                    # Drop sampling-only knobs so validation passes
                    clean = gc.to_dict()
                    for k in ("temperature", "top_p", "typical_p", "top_k"):
                        clean.pop(k, None)

                    GenerationConfig.from_dict(clean).save_pretrained(training_args.output_dir)
                else:
                    gc.save_pretrained(training_args.output_dir)
